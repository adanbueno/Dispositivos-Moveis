...
public class GetTransferUser extends Thread {
    Payment activity;
    ListView listView;
    Transfer[] transfers;
    LoggedUser loggedUser;

    public GetTransferUser(Payment payment, ListView listView) {
        ...
        try {
            ...
            if(responseCode >= 400) {
                ...
                activity.runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        Toast.makeText(activity, "Error transfers", Toast.LENGTH_LONG).show();
                    }
                });
            } else {
